...
const squaresize = 15; //square size, smaller > more squares > longer calculating times because my method is a bit inneficient
const canvas = document.getElementById("canvas");
let wh, t_id, lastSq, canvGlobal;
setSizes();
canvas.width = canvas.clientWidth;
canvas.height = canvas.clientHeight;
const squaresCanvas = canvas.getContext("2d");
const row = canvas.width / squaresize;
let pos = { x: 0, y: 0 };
let arrPos = 1;
let squaresArr = ["uwu"];
let shown = false;
...
class square {
    ...
    makeSquare(color) {
        squaresCanvas.fillStyle = color;
        squaresCanvas.fillRect(this.info.x, this.info.y, this.size, this.size);
        squaresCanvas.strokeStyle = "white";
        squaresCanvas.lineWidth = 0.2;
        squaresCanvas.strokeRect(this.info.x, this.info.y, this.size, this.size);
        squaresCanvas.closePath();
    }
}
...
while (pos.y < canvas.height) {
    while (pos.x < canvas.width) {
        squaresArr[arrPos] = new square(pos.x, pos.y, "black");
        squaresArr[arrPos].makeSquare();
        arrPos++;
        pos.x += squaresize;
    }
...
function start() {
    if (!started) {
        ...
        let toWhite = [];
        //let newGrid = [];
        for (let i = 1; i < squaresArr.length; i++) {//numbers of squares to check
            ...
            */
            ...
            if (squaresArr[i].info.x === 0) {//left
                check = [{ 1: i - row, 2: i - (row - 1), 3: i + 1, 4: i + row, 5: i + (row + 1) }];
            } else if (squaresArr[i].info.x === wh - squaresize) {//right
            ...
            for (let j = 0; j < Object.keys(check[0]).length; j++) {
                if (squaresArr[check[0][Object.keys(check[0])[j]]]?.info?.color === "white") num++;
            }

            //saves info to make it alive/dead
            if ((squaresArr[i].info.color === "white" && (num === 3 || num === 2)) || (squaresArr[i].info.color === "black" && num === 3)) {
                toWhite.push(i);
        ...
        for (let i = 0; i < toWhite.length; i++) squaresArr[toWhite[i]].info.color = "white";

        for (let i = 0; i < toBlack.length; i++) squaresArr[toBlack[i]].info.color = "black";
...
function reloadCanvas(num) {
    if (/[1-9]+/.test(num)) {
        squaresCanvas.clearRect(squaresArr[num].info.x, squaresArr[num].info.y, squaresize, squaresize);
        squaresArr[num].makeSquare(squaresArr[num].info.color);
    } else {
        squaresCanvas.clearRect(0, 0, canvas.width, canvas.height);
        for (let i = 1; i < squaresArr.length; i++) squaresArr[i].makeSquare(squaresArr[i].info.color);
    }
}

//sets sizes and positions for different objects depending on screen size (this is before i knew media queries)
function setSizes() {
    ...
    } else document.documentElement.style.setProperty(`--h1`, `${canv.width + canv.left * 3}px`);
}
...
//sets square color to white and recolors it, then reloads the canvas
canvas.addEventListener("click", (e) => {
    const x = e.x - canvGlobal.left;
    const y = e.y - canvGlobal.top;

    for (let i = 1; i < squaresArr.length; i++) {
        if (x > squaresArr[i].info.x && x <= squaresArr[i].info.x + squaresize && y > squaresArr[i].info.y && y <= squaresArr[i].info.y + squaresize) {
            if (squaresArr[i].info.color === "black") squaresArr[i].info.color = "white";
            else squaresArr[i].info.color = "black";
...
//recolors square if the mouse moves over it and is pressed down
